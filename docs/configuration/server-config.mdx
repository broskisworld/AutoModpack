...
| `lanPeerSharingEnabled`  | `false`                                                    | Lets connected, authorized players see each other's LAN address so they can download modpack files directly from one another. Off by default because it discloses players' LAN addresses to each other. See [LAN Peer Sharing](../technicals/lan-peer-sharing). |
